...
- **Prod migrations now auto-apply; never let Client/DB drift (incident 2026-06-13).** The Vercel build runs `prisma migrate deploy` **on production builds only** (`app/web/vercel.json`, guarded by `VERCEL_ENV=production`; preview skips it — Preview env has no DB creds). Root cause of the incident: build ran `prisma generate` but never `migrate deploy`, so the schema change `Post.classificationAttempts` (migration `20260612120000`) shipped to prod with the Client writing a column the DB never got → **every `POST /api/posts/create` 500'd with `column "classificationAttempts" does not exist` → "An unexpected error occurred"**. Lesson: an `@default()` scalar field makes Prisma include the column in every INSERT, so adding one to the schema + deploying *without* applying the migration breaks all writes to that table. If you ever hit a generic 500 right after a schema change, run `npx prisma migrate status` against prod first. Prod DB = single Supabase project `clumuinbirdlslzdicnx`; migrate via `DIRECT_URL` (port 5432), never the pooler.
