...
> **Last updated:** 2026-06-08 — Story Sharing Phase 1 shipped. `/api/stories/generate` live. `StoryShareButton` integrated into upload completion flow.
...
| 8E | Story Sharing Phase 1 | ✅ COMPLETE — shipped (2026-06-08) | Slice 8B |
...
- **Slice 8E (Story Sharing Phase 1) — ✅ SHIPPED (2026-06-08):** Viral acquisition loop. `GET /api/stories/generate?postId=X` generates a 1080×1920 PNG story card server-side using `next/og` (Satori/ImageResponse). Card includes: user workout photo (hero/cinematic), plant emoji at current evolution level with green glow, streak count, plant name, personal best indicator, streak progress dots, BeActive branding. Auth: session cookie; access control: requestor must own the post, post must be VERIFIED (403/404 otherwise). Fonts: Plus Jakarta Sans Bold + ExtraBold loaded from Google Fonts CDN, module-level cached. `StoryShareButton` client component with loading/error states; uses Web Share API `navigator.share({ files })` for native OS share sheet (Instagram Stories / Snapchat / Telegram); falls back to `<a download>` on desktop. Button integrated into `recorded` stage of `upload/page.tsx`, replacing the old plain-text share. Architecture spec: `docs/BeActive Story Sharing.md`. **636 tests pass.** Phase 2 (animated) and Phase 3 (Wrapped) documented in spec — not built.
